...
export class SancionService {
  ...
  list(filtros?: { id_usuario?: string, id_prestamo?: string }): Observable<SancionRead[]> {
    let params = new HttpParams().set('skip', 0).set('limit', 500);
    
    if (filtros?.id_usuario) {
      params = params.set('por_usuario', filtros.id_usuario);
    }
    if (filtros?.id_prestamo) {
      params = params.set('por_prestamo', filtros.id_prestamo);
    }
